...
if __name__ == '__main__':
    ...
    for R in R_values:
        ...
        hp = Hyperband(objective=url_evaluator, classifier_path=classifier_path, x=X_test_botnet[:BATCH_SIZE], y=y_test_botnet[:BATCH_SIZE], sampler=sampler, eps=eps, dimensions=dimensions, max_configuration_size=len(mutables)-1, R=R, downsample=downsample, distance=distance, seed=seed)
        profiler = cProfile.Profile()
        profiler.enable()
        scores, configs, candidates, _, _, _ = hp.generate(mutables=mutables, features_min_max=(min_constraints,max_constraints), int_features=int_features, scaler=scaler)
        profiler.disable()
        ...
        model_pipeline = Pipeline(steps=[('preprocessing', scaler), ('model', wrap_model(model, X_test_botnet[:BATCH_SIZE], model_task='classification'))])
        
        success_rate_calculator = TorchCalculator(classifier=model_pipeline, data=X_test_botnet[:BATCH_SIZE], labels=y_test_botnet[:BATCH_SIZE], scores=np.array(scores), candidates=candidates, scaler=scaler)
